...
4. Calculate compound PPG retention indices
5. Save matching results and retention indices to a new Excel table
Author: Qianlei Yao
Version: 8.0
Date: 2025
"""
...
class MzMLCompoundMatcherGUI:
    ...
    def setup_ui(self):
        ...
        # Version
        version_label = tk.Label(content_frame, text="Version 8.0 - compound matchingPPG retention indicescalculate",
                                 font=("Arial", 10))
        ...
        ttk.Label(step5_frame, textvariable=self.progress_var).pack(anchor=tk.W, pady=(0, 5))

        self.progress_bar = ttk.Progressbar(step5_frame, mode='determinate', length=950)
        self.progress_bar.pack(fill=tk.X, pady=(0, 5))

        self.progress_percent = tk.StringVar(value="0%")
        ttk.Label(step5_frame, textvariable=self.progress_percent).pack(anchor=tk.E)

        # ==================== 6: ====================
        step6_frame = tk.LabelFrame(content_frame, text="6: ", font=("Arial", 12, "bold"),
                                    padx=10, pady=10)
        step6_frame.pack(fill=tk.BOTH, expand=True, pady=(0, 20))

        # translated note
        self.log_text = scrolledtext.ScrolledText(step6_frame, width=130, height=15,
    ...
    def start_processing(self):
        ...
        if source == "mzml":
            ...
            self.processing_thread = threading.Thread(
                ...
                args=(mzml_file, compound_file, output_dir, enable_ppg,
            ...
            self.processing_thread = threading.Thread(
                ...
                args=(csv_file, compound_file, output_dir, enable_ppg,
                      ppm_tolerance, rt_window, intensity_threshold,
                      ppg_file, ppg_method)
            )
    ...
    def process_mzml_match(self, mzml_file: str, compound_file: str, output_dir: str,
                           enable_ppg: bool, ppm_tolerance: float, rt_window: float,
                           intensity_threshold: float, ppg_file: str, ppg_method: str):
        ...
        try:
            ...
            self.process_compound_matching(peaks_df, compound_file, output_dir,
                                           enable_ppg, ppm_tolerance, rt_window,
                                           mzml_file, ppg_file, ppg_method)
    ...
    def process_compound_matching(self, peaks_df: pd.DataFrame, compound_file: str,
                                  output_dir: str, enable_ppg: bool,
                                  ppm_tolerance: float, rt_window: float,
                                  source_file: str, ppg_file: str, ppg_method: str):
        """compound matching"""
        try:
...
if __name__ == "__main__":
    ...
    print(" 5. Calculate compound PPG retention indices")
    print(" 6. matching resultsretention_indexExcelfile")
    print("=" * 70)
